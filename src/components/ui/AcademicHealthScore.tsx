  ...
  return (
    <div className="relative flex items-center justify-center w-36 h-36 sm:w-40 sm:h-40 shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-3xl sm:text-5xl font-black text-zinc-900 dark:text-zinc-50 tracking-tight tabular-nums">
          {score}
        </span>
        <span className="text-[9px] sm:text-[10px] font-bold text-zinc-400 dark:text-zinc-500 tracking-widest uppercase mt-0.5">
          Score Geral
        </span>
      </div>
    </div>
...
function TrendSparkline({ data }: { data: number[] }) {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} className="shrink-0 opacity-80 group-hover:opacity-100 transition-opacity hidden sm:block">
      <polyline
        fill="none"
        stroke={stroke}
        strokeWidth={2}
...
function BreakdownBar({
  ...
  return (
    // RESPONSIVIDADE: Otimizado o padding interno para telas muito pequenas
    <div className="group space-y-1.5 p-1.5 sm:p-2.5 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors border border-transparent hover:border-zinc-100 dark:hover:border-zinc-800/50">
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-zinc-200 transition-colors truncate">
          {label}
        </span>
      ...
      <div className="h-1.5 sm:h-2 rounded-full bg-zinc-100 dark:bg-zinc-800/80 overflow-hidden relative">
        <div
...
function InsightText({ breakdown, sparklines }: { breakdown: Breakdown; sparklines: Record<string, number[]> | null }) {
  ...
  return (
    <div className="flex items-start gap-2.5">
      {/* <div className="w-5 h-5 rounded-md bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center shrink-0 mt-0.5 text-zinc-500 hidden sm:flex">
        <Sparkles size={12} />
      </div> */}
      <p className="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
...
export default function AcademicHealthScore() {
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800/80 shadow-sm p-5 sm:p-8 space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 sm:gap-8">
          <div className="w-36 h-36 sm:w-40 sm:h-40 rounded-full bg-zinc-100 dark:bg-zinc-800/60 animate-pulse shrink-0" />
  ...
  if (error || !data) {
    return (
      <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-5 sm:p-8">
  ...
  return (
    <div className={cn(
      "bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800/80 shadow-sm p-5 sm:p-8 relative overflow-hidden bg-gradient-to-tr", 
      colors.glow
    )}>
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8 relative z-10">
        <div className="flex items-center gap-3">
          <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center border shrink-0", colors.iconBg)}>
        ...
        <button className="flex items-center justify-center gap-1.5 text-xs font-semibold text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors bg-zinc-50 dark:bg-zinc-800/60 px-2.5 py-1.5 rounded-lg border border-zinc-100 dark:border-zinc-800/60 shadow-sm w-full sm:w-auto">
          Relatório completo
          <ArrowRight size={12} />
        </button>
      </div>
      ...
      {/* Body */}
      <div className="flex flex-col md:flex-row items-center md:items-start gap-6 sm:gap-8 lg:gap-12 relative z-10">
        
        {/* Left Area: Anel de Score + Status */}
        <div className="flex flex-col items-center gap-4 shrink-0 bg-zinc-50/50 dark:bg-zinc-800/20 border border-zinc-100/70 dark:border-zinc-800/30 p-4 rounded-2xl w-full md:w-auto md:min-w-[190px]">
          <ScoreRing score={visible ? displayScore : 0} status={data.status} />
          
          <div className="flex flex-col items-center gap-1.5 w-full text-center">
            <span className={cn("text-xs font-bold px-3 py-1 rounded-full border shadow-2xs tracking-wide w-full max-w-[140px]", colors.badgeBg)}>
              Status: {colors.label}
            </span>
        ...
        {/* Right Area: Listas de Critérios (Grid Adaptativo) */}
        <div className="flex-1 w-full space-y-4 sm:space-y-5 min-w-0">
          {/* RESPONSIVIDADE: Grelha altera dinamicamente dependendo da largura do dispositivo */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-1 gap-1.5 sm:gap-3">
            {breakdowns.map((b, i) => (
              <div
                key={b.key}
          ...
          {sparklines && (
            <div
              className="p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-100 dark:border-zinc-800/80 transition-all duration-700 shadow-2xs w-full"
              style={{
                opacity: visible ? 1 : 0,
                transform: visible ? "translateY(0)" : "translateY(10px)",
