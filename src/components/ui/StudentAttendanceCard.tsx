...
const StudentAttendanceCard = ({ studentId }: Props) => {
  ...
  if (!data || data.total === 0) return <div className="text-sm text-muted-foreground text-center py-3">Sem registos</div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-foreground">Assiduidade</span>
      ...
      <p className="text-[11px] text-muted-foreground text-center">{data.total} aulas registadas</p>
    </div>
  )
}
