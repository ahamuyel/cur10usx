    ...
    <div className="w-full space-y-8 animate-fade-in pb-20">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-70">Aula em Curso</span>
                  </div>
                ...
                <div className="flex flex-col items-start md:items-end gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-60">Tempo Restante</span>
                  <div className="text-4xl font-black tabular-nums tracking-tighter">
                    {moment(current.endTime, "HH:mm").diff(moment(now), 'minutes')} min
                  </div>
                </div>
              </div>
            </motion.div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-violet-600 dark:text-violet-400">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Próxima Aula</span>
                  </div>
                ...
                <div className="flex flex-col items-start md:items-end gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Inicia em</span>
                  <div className="text-4xl font-black tabular-nums tracking-tighter text-zinc-900 dark:text-white">
                    {moment(next.startTime, "HH:mm").diff(moment(now), 'minutes')} min
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-8">
          {/* 2. MICRO-MÉTRICAS DO DIA */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-zinc-900/40 p-5 rounded-3xl border border-zinc-100 dark:border-zinc-800/60 shadow-xs">
              <div className="flex items-center gap-2 mb-3">
                <div className="p-2 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-blue-600 dark:text-blue-400">
                  <BookOpen size={16} />
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Aulas de Hoje</span>
              </div>
            ...
            <div className="bg-white dark:bg-zinc-900/40 p-5 rounded-3xl border border-zinc-100 dark:border-zinc-800/60 shadow-xs">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Próxima Prova</span>
              </div>
              <div className="text-xs font-semibold text-zinc-900 dark:text-white truncate pt-1">
                {dashboardData?.upcomingExams[0]?.title || "Nenhuma agendada"}
              </div>
            </div>

            <div className="bg-white dark:bg-zinc-900/40 p-5 rounded-3xl border border-zinc-100 dark:border-zinc-800/60 shadow-xs">
              <div className="flex items-center gap-2 mb-3">
                <div className="p-2 bg-violet-50 dark:bg-violet-900/20 rounded-xl text-violet-600 dark:text-violet-400">
                  <ClipboardList size={16} />
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Pendentes</span>
              </div>
              <div className="text-2xl font-black text-zinc-900 dark:text-white tabular-nums">
        ...
        <aside className="lg:col-span-4 space-y-8">
          <div className="bg-zinc-50 dark:bg-zinc-900/40 rounded-[2rem] p-6 border border-zinc-100 dark:border-zinc-800/60 shadow-xs">
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-6">Agenda de Avaliações</h3>
            
            <div className="space-y-6">
              {/* EXAMS */}
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-zinc-400 uppercase">Provas</span>
                ...
                {dashboardData?.upcomingExams && dashboardData.upcomingExams.length > 0 ? (
                  dashboardData.upcomingExams.slice(0, 3).map(exam => (
                    <div key={exam.id} className="group relative bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800/80 shadow-2xs hover:border-rose-500/30 dark:hover:border-rose-500/20 transition-all duration-200 cursor-pointer">
                      <p className="text-xs font-semibold text-zinc-900 dark:text-zinc-100 mb-1 line-clamp-1">{exam.title}</p>
                      <div className="flex items-center justify-between text-[10px] font-medium text-zinc-400 dark:text-zinc-500">
                        ...
                        <span className="tabular-nums">{moment(exam.date).format("DD MMM")}</span>
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-[11px] text-zinc-400 italic pl-1">Sem exames marcados.</p>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-zinc-400 uppercase">Tarefas</span>
                ...
                {dashboardData?.upcomingAssignments && dashboardData.upcomingAssignments.length > 0 ? (
                  dashboardData.upcomingAssignments.slice(0, 3).map(task => (
                    <div key={task.id} className="group relative bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800/80 shadow-2xs hover:border-violet-500/30 dark:hover:border-violet-500/20 transition-all duration-200 cursor-pointer">
                      <p className="text-xs font-semibold text-zinc-900 dark:text-zinc-100 mb-1 line-clamp-1">{task.title}</p>
                      <div className="flex items-center justify-between text-[10px] font-medium text-zinc-400 dark:text-zinc-500">
                        <span>{task.subjectName}</span>
                        <span className="tabular-nums">{moment(task.dueDate).format("DD MMM")}</span>
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-[11px] text-zinc-400 italic pl-1">Sem tarefas pendentes.</p>
                )}
              </div>
            </div>

            <button className="w-full mt-8 py-3.5 rounded-2xl bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-[10px] font-bold uppercase tracking-widest hover:opacity-95 transition-all flex items-center justify-center gap-2 border border-transparent active:scale-[0.99]">
              Ver Agenda Completa
