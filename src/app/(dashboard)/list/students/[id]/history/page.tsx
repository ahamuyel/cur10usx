...
export default function StudentHistoryPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const sorted =
          json.currentYear?.subjects?.sort(
            (a: HistorySubject, b: HistorySubject) =>
              b.totalAverage - a.totalAverage,
          ) || [];
        setData({
          ...json,
          currentYear: { ...json.currentYear, subjects: sorted },
        });
      } catch (e) {
  ...
  const filteredSubjects = useMemo(() => {
    ...
    return data.currentYear.subjects.filter((s) => {
      ...
      if (selectedTrimester !== "all") {
        const hasResults = s.results.some(
          (r) => (r.trimester || "primeiro") === selectedTrimester,
        );
  ...
  const filteredTimeline = useMemo(() => {
    if (!data) return [];
    return data.currentYear.timeline
      .map((m) => ({
        ...m,
  ...
  const bestSubject =
    subjects.length > 0
      ? subjects.reduce((a, b) => (a.totalAverage >= b.totalAverage ? a : b))
      : null;
  const worstSubject =
    subjects.length > 0
      ? subjects.reduce((a, b) => (a.totalAverage <= b.totalAverage ? a : b))
    ...
    <div className="w-full space-y-8 pb-16 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <SummaryCard
          icon={FileText}
          label="Avaliações"
          value={`${student.totalAssessments}`}
        />
        <SummaryCard
          icon={Users}
          label="Faltas"
        ...
        <div className="bg-white dark:bg-zinc-900/40 rounded-3xl border border-zinc-100 dark:border-zinc-800/60 p-5">
          ...
          <div className="flex items-end gap-3 h-24">
            {trimesterEvolution.map((t, i) => {
              const max = Math.max(
                ...trimesterEvolution.map((x) => x.generalAverage),
                20,
              );
              const height = (t.generalAverage / max) * 100;
              const prev =
                i > 0
                  ? t.generalAverage - trimesterEvolution[i - 1].generalAverage
                  : 0;
              return (
                <div
                  key={t.trimester}
                ...
                >
                  ...
                  <span className="text-[9px] text-zinc-400">
                    {t.label.replace("Trimestre", "Trim")}
                  </span>
                </div>
              );
            })}
        ...
        {filteredSubjects.map((subject) => {
          ...
          const triAvgs = trimesterOrder
            ...
            .map((t) => ({
              ...
              average: subject.trimesterAverages[t]!,
            }));
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        scoreColor(subject.totalAverage),
                      )}
                    >
                      ...
                      <div className="flex flex-wrap gap-4 text-[11px] text-zinc-500 dark:text-zinc-400">
                        ...
                        {subject.totalLessons > 0 && (
                          <div className="text-zinc-400">
                            <span>
                              {subject.absences} falta
                              {subject.absences !== 1 ? "s" : ""} em{" "}
                              {subject.totalLessons} aula
                              {subject.totalLessons !== 1 ? "s" : ""}
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      {triAvgs.length > 1 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {triAvgs.map((t, i) => {
                              const prev =
                                i > 0 ? t.average - triAvgs[i - 1].average : 0;
                              return (
                                <div
                                  key={t.trimester}
                                  className="flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-50 dark:bg-zinc-800/20 border border-zinc-100 dark:border-zinc-800/40"
                                >
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {subject.absenceDates.map((d, i) => (
                              <span
                                key={i}
                                className="text-[10px] font-medium text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10 px-2 py-1 rounded-md border border-rose-100 dark:border-rose-900/20"
                              >
