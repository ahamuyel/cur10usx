...
import { createCurriculumTopicSchema } from "@/lib/validations/academic"
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const unit = await prisma.curriculumUnit.findUnique({ where: { id } })
    if (!unit) {
    ...
    const [data, total] = await Promise.all([
      ...
      prisma.curriculumTopic.count({ where }),
    ])
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = createCurriculumTopicSchema.safeParse({ ...body, curriculumUnitId: id })
